 ...
 *
 * Erase all data from the Nexus NV partition (keycode history, Channel state,
 * etc).
 *
...
/**
 * @brief flash_filesystem_erase_product_nv
 *
 * Erase all data from the Product NV partition (PAYG credit, etc).
 *
 * Can be used to 'reset' the device product state in testing.
 *
 * @retval -ERRNO errno code if error
 */
