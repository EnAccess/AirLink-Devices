...
// Confirm the pushbutton node is 'okay' (ready to use)
    ...
    #define PAYG_LED_GPIO_FLAGS                                                \
        (GPIO_OUTPUT_ACTIVE | DT_GPIO_FLAGS(PAYG_LED_NODE, gpios))
#else
...
void payg_led_display_init(void)
{
    ...
    int ret = gpio_pin_configure(
        _this.payg_led_dev, PAYG_LED_GPIO_PIN, PAYG_LED_GPIO_FLAGS);
    assert(ret == 0);
