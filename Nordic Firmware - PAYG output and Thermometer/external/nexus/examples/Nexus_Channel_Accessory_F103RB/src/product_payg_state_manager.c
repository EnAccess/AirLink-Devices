...
static void _product_payg_state_manager_trigger_payg_led_update(uint32_t new_credit)
{
    if (new_credit == 0)
    {
        payg_led_display_solid_off();
    }
    // PAYG unlocked
    else if (new_credit == PRODUCT_PAYG_STATE_MANAGER_UNLOCKED_CREDIT_SENTINEL)
    {
        payg_led_display_solid_on();
    }
    // PAYG enabled
    else
    {
        payg_led_display_begin_blinking();
    }
}
...
void product_payg_state_manager_init(void)
{
    ...
    _product_payg_state_manager_trigger_payg_led_update(_this.credit_remaining);
}
