    ...
    const struct nx_keycode_complete_code* passthrough_keycode)
{
    ...
    switch (subtype_id)
    {
        case NXP_KEYCODE_PASSTHROUGH_APPLICATION_SUBTYPE_ID_NX_CHANNEL_ORIGIN_COMMAND:
            ...
            // a link.
				///todo: nx_channel_handle_origin_command(NX_CHANNEL_ORIGIN_COMMAND_BEARER_TYPE_ASCII_DIGITS,(void*) &passthrough_keycode->keys[1],length);
...
void nxp_keycode_notify_custom_flag_changed(enum nx_keycode_custom_flag flag,
                                            bool value)
{
		NRF_LOG_INFO("Keycode: Custom flag %d changed to value %d", flag, value);
}
