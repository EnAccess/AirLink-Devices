...
#include <time.h>
#include "app_timer.h"
//APP_TIMER_DEF(m_nexus_tick_timer);
#include "nrf_log.h"
...
void processing_init(void)
{
    ...
    ///timer_create(CLOCK_MONOTONIC, &_this.sev, &_this.timerid);
		//ret_code_t err_code;
		//err_code = app_timer_create(&m_nexus_tick_timer, APP_TIMER_MODE_REPEATED, timer_thread);
		//APP_ERROR_CHECK(err_code);
		//err_code = app_timer_start(m_nexus_tick_timer, 10000, NULL);
		//APP_ERROR_CHECK(err_code);
}
...
void processing_deinit(void)
{
    ...
    ///}
		//app_timer_stop(m_nexus_tick_timer);
}
...
void processing_execute(void)
{
    ...
    {
				// Handled by flash: payg_state_update_nv();
        _this.seconds_since_payg_state_backup = 0;
    }
}
