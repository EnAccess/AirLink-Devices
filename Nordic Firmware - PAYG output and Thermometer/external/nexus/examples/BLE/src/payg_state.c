...
static struct
{
    ...
    uint8_t last_payg_state;					// modified from bool
} _this;

void payg_state_init(void)
{
    // attempt to read from NV
		bool valid_payg_state = true; 
		//done: prod_nv_read_payg_state(sizeof(struct payg_state_struct), (uint8_t*) &_this.stored);
		//Overwritten by local variables
	
    // If we retrieve a valid PAYG state from NV, use it.
    if (valid_payg_state)
    {
        _this.stored.credit = payg.payG_credit_remaining;
				_this.stored.is_unlocked = payg.is_unlocked;
				return;
    }
    ...
    //todo: prod_nv_write_payg_state(sizeof(struct payg_state_struct),(uint8_t*) &_this.stored);
}
...
bool update_payg_state(bool is_unlocked, uint32_t credit)
{
    ...
    if (_this.last_payg_state != current_payg_state)
    {
        ...
        _this.last_payg_state = current_payg_state;
				...
				provision_state_handler(PROVISIONING_STATUS_PAYG_STATE_CHANGED);
				
			NRF_LOG_INFO("payg_state changed:%d",current_payg_state);
    }
...
{
	//prod_nv_write_payg_state(sizeof(struct payg_state_struct),(uint8_t*) &_this.stored);
	if(payg.payG_credit_remaining != _this.stored.credit){
		payg.payG_credit_remaining = _this.stored.credit;
		flash_upload_flag |= NEXUS_PAGE_FLAG;
		NRF_LOG_INFO("payg_state_update_nv2.");
	}
	if(payg.is_unlocked != _this.stored.is_unlocked){
		payg.is_unlocked = _this.stored.is_unlocked;
		flash_upload_flag |= CONFIG_PAGE_FLAG;
		NRF_LOG_INFO("payg_state_update_nv1.");
	}
	//NRF_LOG_INFO("payg_state_update_nv.");
}

bool nxp_keycode_payg_credit_add(uint32_t credit)
{
    payg.last_payg_credit = credit;
		payg.last_payG_update = payg.local_time;
		payg.last_payG_calc = payg.local_time;
