...
void screen_display_status(void)
{
    if (nxp_common_payg_state_get_current() == NXP_COMMON_PAYG_STATE_UNLOCKED)
			{
				if(state != 0xffffff)
					NRF_LOG_INFO("The device is unlocked\n");
				state = 0xffffff;
			}
			else
			{
				if(state != payg_state_get_remaining_credit()){
					payg.payG_credit_remaining = payg_state_get_remaining_credit();
					NRF_LOG_INFO("Credit remaining: %d seconds\n",payg_state_get_remaining_credit());
				}
				state = payg_state_get_remaining_credit();
			}
}
