...
extern "C" {
    ...
    // controller will keep trying to reach an accessory and complete a
    // handshake for this long. Allows user time to connect accessory before
    // timeout. (1 hour)
...
// Represents one in-progress handshake from the client/controller
// perspective and the MAC should be delegated up to security manager based
// on that link
typedef struct nexus_link_hs_controller_state_t
{
    ...
    // or struct, but for simplicity with one challenge mode, keep them
    // here.
 ...
 *
 * For challenge mode 0, takes the salt from the received challenge data,
 * and iterate through handshake counts, generating link keys using the
 * known key derivation formula (using the handshake count and salt as
 * input).
 *
 * For each key, attempt to compute a MAC over the salt. If the MAC matches
 * the MAC which was transmitted in the challenge data with this salt,
 * return true and update the correct handshake/challenge count via
 * reference.
 *
