...
allocate_events(void)
{
  ...
  for (i = 0; i < __NUM_OC_EVENT_TYPES__; i++) {
    ...
    // PRINT("new event type: oc_events[%d] = %#04x\n", i, oc_events[i]);
  }
}
...
oc_ri_delete_all_app_resources(void)
{
  oc_resource_t *res = oc_ri_get_app_resources();
  while (res) {
    OC_DBG("Deleting a resource %s", oc_string(res->types));
...
/* no callsites - observe not implemented, callsites commented
...
*/
  ...
  if (!bad_request) {
    int i;
    // Core resources like /oic/d, /oic/p, etc.
    // uri_path may not be null terminated
    //OC_DBG("Checking %d core resources for URI=%s handler", OC_NUM_CORE_RESOURCES_PER_DEVICE, uri_path);
  ...
  if (!cur_resource && !bad_request) {
    // uri_path may not be null terminated
    //OC_DBG("App resource exists for URI=%s in device %lu?", uri_path, endpoint->device);
...
*/
  ...
  if (cur_resource) {
    ...
    if (iface_mask == 0)
    {
      OC_DBG("Using default interface");
    ...
    if (((iface_mask & ~cur_resource->interfaces) != 0) ||
      ...
      OC_WRN("Selected interface not supported/invalid");
      forbidden = true;
      bad_request = true;
    }
  }
...
// Alloc response_state. It also affects request_obj.response.
  ...
  if (cur_resource && !bad_request) {
     ...
     */
    OC_DBG("Allocating %d byte buffer for response", response_buffer.buffer_size);
    oc_rep_new(response_buffer.buffer, response_buffer.buffer_size);

/*
// XXX we can use this pattern for Nexus security - query the ACL here.
// Not in an ifdef, just 'check ACL for this device' and ensure its there
#ifdef OC_SECURITY
 ...
 */
  ...
  OC_WRN("Separate response not currently supported");
  } else
#endif // OC_SERVER
...
*/
      ...
      response_buffer.code < oc_status_code(OC_STATUS_BAD_REQUEST))
      {
          ...
          // No observers to notify, OBSERVE not supported
      }
  ...
  OC_DBG("Request handled? %d", success);
  ...
  if (payload_len) {
    if (cb->discovery) {
      ...
      if (oc_ri_process_discovery_payload(payload, payload_len, cb->handler,
        ...
        OC_WRN("Discovery is not currently supported");
        uint16_t mid = cb->mid;
        cb->ref_count = 0;
    ...
    if (pkt->type == COAP_TYPE_ACK && pkt->code == 0) {
      separate = true;
      cb->separate = 1;
      OC_WRN("Separate responses are currently unsupported");
    ...
    if (cb->observe_seq == 0) {
      ...
      while (dup_cb != NULL) {
        if (dup_cb != cb && dup_cb->observe_seq != -1 &&
          ...
          OC_DBG("Freeing cb %s, token 0x%02X", oc_string(dup_cb->uri),
                 dup_cb->token[0]);
                      ...
                      void *user_data)
{
  ...
  cb->token_len = 1;
  // Take one byte from random value
  cb->token[0] = oc_random_value() & 0xff;
  cb->discovery = false;
